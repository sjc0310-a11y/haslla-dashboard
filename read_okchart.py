...
# 원장별 일별 매출 (건보/자보/비급여)
#   기존엔 Receipt.sn ↔ Detail.SN으로 JOIN했는데 두 컬럼 포맷이 다르고
#   (Receipt.sn = '016058', Detail.SN = '16972' 같이) 매칭률이 주마다 들쭉날쭉.
#   환자(Customer_PK)+날짜만으로 매칭하고, 그 환자의 그 날 첫 진료 원장
#   (Detail_PK ASC 첫째)에 매출 귀속.
SQL_REVENUE = """
SELECT
    CONVERT(DATE, r.TxDate)                                  AS 날짜,
    doc.TxDoctor                                             AS 원장명,
    r.Calcu_Type                                             AS 구분,
    SUM(CAST(ISNULL(r.Bonin_Money, 0)   AS BIGINT)
      + CAST(ISNULL(r.CheongGu_Money, 0) AS BIGINT))         AS 보험매출,
