...
.metric-add { display:flex; gap:6px; align-items:center; margin-top:8px;
...
.week-box-header::-webkit-details-marker { display:none; }
.week-box-header::before { content:"▶"; font-size:9px; transition:transform .2s;
                            color:var(--accent); margin-right:4px; }
.week-box[open] > .week-box-header { padding-bottom:7px;
                                       border-bottom:1px solid var(--border);
                                       margin-bottom:8px; }
.week-box[open] > .week-box-header::before { transform:rotate(90deg); }
.week-box:not([open]) > .week-box-header { padding-bottom:4px; }
...
function openProjectModal(doc, pid) {
  ...
  const renderWeekBox = (w) => {
    ...
    // 현재 주는 자동 펼침, 나머지는 접힘
    return `
      <details class="week-box ${isCurrent?'current':''} ${total===0?'empty':''}" ${isCurrent?'open':''}>
        <summary class="week-box-header">
