...
/* 3 질문 모드 — 단순 흐름 */
.agenda-simple { display:flex; flex-direction:column; gap:18px; margin-top:18px; }
.agenda-block { background:var(--panel); border:1px solid var(--border);
                 border-radius:10px; padding:14px 16px; }
.agenda-block .agenda-label { font-size:15px; font-weight:700; color:var(--text);
                                margin-bottom:8px; }
.agenda-block .agenda-label .hint { font-size:12px; color:var(--muted);
                                      font-weight:400; margin-left:8px; }
textarea.agenda-big { min-height:140px; font-size:15px; line-height:1.7; }
/* 옛 Work / Career (롤백 대비, 현재 미사용) */
...
.ck-week { background:var(--panel3); color:var(--accent); border:1px solid var(--border);
...
// 빈 면담에 디폴트 포맷 자동 채움 — 화면 그릴 때마다 호출
function ensureMeetingTemplate(m) {
  if (!m || READONLY) return m;
  let changed = false;
  if (!m.reflect)      { m.reflect = REFLECT_TEMPLATE; changed = true; }
  if (!m.next)         { m.next = NEXT_TEMPLATE; changed = true; }
  if (!m.support_text) { m.support_text = SUPPORT_TEMPLATE; changed = true; }
...
function currentMeeting(doc) {
  ...
  currentIdx[doc] = idx;
  return ensureMeetingTemplate(notes[idx]);
}
...
// 면담 내용 있는지·총 글자수
function meetingHasContent(m) { return !!((m.reflect||"") || (m.next||"")); }
function meetingCharCount(m)  { return (m.reflect||"").length + (m.next||"").length; }
...
// ───── 이번 회차 점검 위젯 — 직전 면담 이후 ~ 이번 면담까지의 W 액션 노출 ─────
function renderMeetingChecklist(doc) {
  const notes = getNotes(doc);
  const idx = currentIdx[doc] ?? (notes.length - 1);
  const curMeeting  = notes[idx];
  const prevMeeting = idx > 0 ? notes[idx-1] : null;
  const dayMs = 24*60*60*1000;
  const curDate  = curMeeting?.date  ? new Date(curMeeting.date)  : new Date();
  const prevDate = prevMeeting?.date ? new Date(prevMeeting.date) : null;

  // 각 프로젝트별 점검할 W 범위 계산 (직전 면담 다음 주부터 이번 면담 주차까지)
  function weekRangeFor(p) {
    if (!p.start_date) return null;
    const start = new Date(p.start_date);
    const curWkRaw = Math.floor((curDate - start)/dayMs/7) + 1;
    const curWk = Math.max(1, Math.min(12, curWkRaw));
    let fromWk;
    if (prevDate && prevDate >= start) {
      const prevWkRaw = Math.floor((prevDate - start)/dayMs/7) + 1;
      fromWk = Math.max(1, Math.min(12, prevWkRaw + 1));   // 직전 면담 다음 주부터
    } else {
      fromWk = 1;   // 첫 면담이면 처음부터
    }
    if (fromWk > curWk) fromWk = curWk;   // 안전
    return { from: fromWk, to: curWk, curWk: curWkRaw };
  }
  ...
  if (projects.length === 0) {
    return `
      <details class="meeting-checklist" id="meetingChecklist-${doc}" open>
        <summary class="checklist-summary">🎯 이번 회차 점검 — 직전 면담 이후 주간 액션</summary>
  ...
  const blocks = projects.map(p => {
    if (!p.start_date) {
      return `
        <div class="checklist-block">
          ...
          <div class="meta" style="font-size:12px; padding:4px 8px;">12주 시작일 설정 시 자동 노출</div>
        </div>`;
    }
    const range = weekRangeFor(p);
    if (range.curWk < 1) {
      return `
        <div class="checklist-block">
          <div class="checklist-proj" data-pid="${p.id}">
            <span class="proj-name">📦 ${escapeHtml(p.name||"(이름 없음)")}</span>
            <span class="proj-meta">시작 전 (D${range.curWk})</span>
          </div>
        </div>`;
    }
    if (range.curWk > 12) {
      return `
        <div class="checklist-block">
          <div class="checklist-proj" data-pid="${p.id}">
            <span class="proj-name">📦 ${escapeHtml(p.name||"(이름 없음)")}</span>
            <span class="proj-meta">12주 종료</span>
          </div>
        </div>`;
    }
    // range.from ~ range.to 의 모든 액션 노출 (완료·미완료 다)
    const tactics = (p.tactics||[]).filter(t => t.week >= range.from && t.week <= range.to)
                                     .sort((a,b) => (a.week||0)-(b.week||0));
    const rangeLabel = range.from === range.to
      ? `W${range.from}`
      : `W${range.from}~W${range.to} · ${range.to - range.from + 1}주분`;
    if (tactics.length === 0) {
      return `
        <div class="checklist-block">
          <div class="checklist-proj" data-pid="${p.id}">
            <span class="proj-name">📦 ${escapeHtml(p.name||"(이름 없음)")}</span>
            <span class="proj-meta">${rangeLabel}</span>
          </div>
          <div class="meta" style="font-size:12px; padding:4px 8px;">이 기간에 계획된 액션 없음 — 카드 클릭해서 추가</div>
        </div>`;
    }
    const items = tactics.map(t => `
      <li data-tid="${t.id}" data-pid="${p.id}" class="checklist-item ${t.done ? 'done' : ''}">
        ...
        <span class="ck-week">W${t.week}</span>
    ...
    return `
      <div class="checklist-block">
        ...
        <ul class="checklist-tactics">${items}</ul>
      </div>`;
  }).join("");

  // 통계 (이번 점검 범위의 모든 액션)
  const allTactics = projects.flatMap(p => {
    if (!p.start_date) return [];
    const r = weekRangeFor(p);
    if (!r || r.curWk < 1 || r.curWk > 12) return [];
    return (p.tactics||[]).filter(t => t.week >= r.from && t.week <= r.to);
  });
  const ckDone = allTactics.filter(t => t.done).length;
  const ckTotal = allTactics.length;
  const ckStat = ckTotal > 0
    ? `<span class="ck-stat ${ckDone===ckTotal?'full':''}"> · ${ckDone}/${ckTotal} · ${Math.round(ckDone/ckTotal*100)}%</span>`
    : '';
  const rangeHint = prevMeeting
    ? `직전 면담(${prevMeeting.date}) 이후 ~ 이번 면담(${curMeeting.date}) 까지`
    : `첫 면담까지의 W 액션 전체`;
  return `
    <details class="meeting-checklist" id="meetingChecklist-${doc}" open>
      <summary class="checklist-summary">
        🎯 이번 회차 점검 — ${rangeHint}${ckStat}
...
function renderMeeting(doc) {
  ...
  $sec.innerHTML = `
    ...
    <div class="agenda-simple">
      <div class="agenda-block">
        <div class="agenda-label">📝 이번 달 어땠어?
          <span class="hint">잘된 것·어려운 것·운영·성장 — 떠오르는 대로 자유롭게</span></div>
        ${READONLY
          ? `<div class="agenda-readonly">${escapeHtml(m.reflect||"(작성 안 됨)")}</div>`
          : `<textarea class="agenda agenda-big" id="reflect-${doc}" placeholder="이번 달 굴러간 일·막힌 일·새로 배운 것·인상적이었던 환자 케이스 등 자유롭게...">${escapeHtml(m.reflect||"")}</textarea>`}
      </div>
      <div class="agenda-block">
        <div class="agenda-label">🚀 다음 달 뭐 해볼까?
          <span class="hint">이번에 시도할 작은 실험 1~2개</span></div>
        ${READONLY
  ...
  if (!READONLY) {
    ...
    // 3 질문 모드 — reflect / next
    const $reflect = document.getElementById(`reflect-${doc}`);
    ...
    : `<ul class="item-list" id="modalMeetings">${touchingMeetings.map(m => {
        if (READONLY) {
          return `<li>
            ...
            <span style="flex:1; color:var(--muted);">${m.done?"✅ 완료":"📝 진행"}${meetingHasContent(m)?" · 작성됨":""}</span>
          </li>`;
        }
        const moodOpts = moodOptionsM.map(([v,t]) => `<option value="${v}" ${m.mood===v?"selected":""}>${t}</option>`).join("");
        return `<li data-mid="${m.id}">
          ...
          <span style="flex:1; color:var(--muted); font-size:11px;">${meetingHasContent(m)?"작성됨":""}</span>
      ...
      : past.map(n => {
          ...
          return `<details>
            <summary>
              ...
              &nbsp;<span class="meta">(${meetingCharCount(n)}자 · support ${(n.support||[]).length}건)</span>
            </summary>
            <div class="body">
              <span class="heading">📝 이번 달 어땠어?</span>
              <p>${escapeHtml(n.reflect||"(없음)")}</p>
              <span class="heading">🚀 다음 달 뭐 해볼까?</span>
              <p>${escapeHtml(n.next||"(없음)")}</p>
