  ...
  <div class="grid3">
    <div class="chart-box">
      ...
      <table class="chuna-table">
        ...
        <thead>
          <tr><th>원장</th><th class="num">건보추나</th><th class="num">TA추나</th><th class="num">합계</th></tr>
        </thead>
        <tbody id="chunaSummaryBody">
          <tr><td colspan="4" style="color:#475569;text-align:center">로딩 중...</td></tr>
        </tbody>
      </table>
    </div>
    <div class="chart-box">
...
function renderWeek(idx) {{
  ...
  for (var i=0; i<chunaDoctors.length; i++) {{
    var c = week.chuna[chunaDoctors[i]] || {{'건보추나':0,'TA추나':0}};
    var row = c['건보추나']+c['TA추나'];
    tG+=c['건보추나']; tT+=c['TA추나'];
    ctbody += '<tr><td class="doc-name">' + chunaDoctors[i] + '</td>' +
      '<td class="num">' + c['건보추나'] + '</td>' +
      '<td class="num">' + c['TA추나'] + '</td>' +
      '<td class="num bold">' + row + '</td></tr>';
  }}
  ctbody += '<tr class="total-row"><td>합계</td>' +
    '<td class="num">' + tG + '</td><td class="num">' + tT + '</td>' +
    '<td class="num bold">' + (tG+tT) + '</td></tr>';
  document.getElementById('chunaSummaryBody').innerHTML = ctbody;

  chunaPieChart.data.datasets[0].data = chunaDoctors.map(function(d) {{
    var c = week.chuna[d]||{{'건보추나':0,'TA추나':0}};
    return c['건보추나']+c['TA추나'];
  }});
  ...
  if (dates.length === 0) {{
    ...
    var dColW = Math.floor(90 / (chunaDoctors.length * 2));
    dthtml = '<table class="chuna-table">' +
      '<colgroup><col style="width:10%">';
    for (var ci=0; ci<chunaDoctors.length*2; ci++) dthtml += '<col style="width:' + dColW + '%">';
    dthtml += '</colgroup><thead><tr><th>날짜</th>';
    for (var i=0; i<chunaDoctors.length; i++) dthtml += '<th class="num" colspan="2">' + chunaDoctors[i] + '</th>';
    dthtml += '</tr><tr><th></th>';
    for (var i=0; i<chunaDoctors.length; i++) dthtml += '<th class="num">건보</th><th class="num">TA</th>';
    ...
    for (var di=0; di<dates.length; di++) {{
      ...
      for (var dj=0; dj<chunaDoctors.length; dj++) {{
        ...
        dthtml += entry
          ? '<td class="num">'+entry['건보추나']+'</td><td class="num">'+entry['TA추나']+'</td>'
    ...
    for (var i=0; i<chunaDoctors.length; i++) {{
      var t = week.chuna[chunaDoctors[i]]||{{'건보추나':0,'TA추나':0}};
      dthtml += '<td class="num">'+t['건보추나']+'</td><td class="num">'+t['TA추나']+'</td>';
    }}
